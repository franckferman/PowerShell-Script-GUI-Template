...
.DESCRIPTION
A ready-to-use template for creating interactive graphical user interfaces (GUIs) with PowerShell, using Windows Forms.
Designed to simplify system management, automation, and administrative tasks without relying on external frameworks like C# or WPF.

.EXAMPLE
PS C:\> Set-ExecutionPolicy -ExecutionPolicy Unrestricted -Scope Process
PS C:\> .\PowerShellScriptGUITemplate.ps1
...
📜 DESCRIPTION:
This is a ready-to-use template for creating interactive GUIs using native PowerShell and Windows Forms.
It simplifies system management, automation, and administrative tasks — without external dependencies.

💻 USAGE:
Simply execute the script to launch the interactive GUI. 
No command-line parameters are required.

🧭 Example to Run the Script:
----------------------------------
PS > Set-ExecutionPolicy -ExecutionPolicy Unrestricted -Scope Process
PS > .\PowerShellScriptGUITemplate.ps1
----------------------------------
...
function Get-MainForm {
    ...
    $Website_Redirect_Button.Add_Click({ [console]::beep(300, 100); Write-Host "Action: Redirection to my website." -ForegroundColor Green; Start-Process -WindowStyle Maximized "https://github.com/franckferman/" })
    ...
    $AboutBtn.Add_Click({
            Start-Process -File '.\assets\audio\fs0ci3ty.mp3' -WindowStyle Minimized
